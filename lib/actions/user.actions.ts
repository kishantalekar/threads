...
export async function fetchUserPosts(userId: string) {
  try {
    ...
    //find all threads author by the user with the given user id
    ...
    throw new Error(`Failed to fetch post of users :${error.message}`);
  }
}
...
export async function getActivity(userId: string) {
  try {
    ...
    throw new Error(`Error in getting Activity :${error.message}`);
  }
}
